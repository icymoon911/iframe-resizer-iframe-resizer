...
test.describe('iframe-resizer Safari compatibility', () => {
  test('iframe height should remain stable without Set.prototype.symmetricDifference', async ({ page }) => {
    // Simulate Safari 16 by removing Set.prototype.symmetricDifference
    ...
    // Wait a bit and check height hasn't changed (no spurious resize messages)
    await page.waitForTimeout(500)
    const heightAfter = await iframeElement.evaluate(el => el.offsetHeight)
    expect(heightAfter).toBe(initialHeight)
  })

  test('fixed-position elements inside iframe should not be treated as hidden', async ({ page }) => {
    await page.goto('/example/html/index.html')
    await page.waitForLoadState('networkidle')

    const iframeElement = page.locator('iframe')

    // Inject a fixed-position element into the iframe content
    await page.evaluate(() => {
      const iframe = document.querySelector('iframe')
      const iframeDoc = iframe.contentDocument || iframe.contentWindow.document
      const fixedEl = iframeDoc.createElement('div')
      fixedEl.style.position = 'fixed'
      fixedEl.style.bottom = '0'
      fixedEl.style.width = '100%'
      fixedEl.style.height = '50px'
      fixedEl.style.background = 'red'
      fixedEl.setAttribute('data-iframe-size', '')
      iframeDoc.body.appendChild(fixedEl)
    })

    // Wait for mutation observer to pick up the change
    await page.waitForTimeout(300)

    // Verify iframe still has a valid height (fixed element should not be ignored)
    const height = await iframeElement.evaluate(el => el.offsetHeight)
    expect(height).toBeGreaterThan(0)
  })
})
